...
class UserAPI(Resource):
    def get(self):
        log_data = db.session.execute(
            db.select(LogUsers.id, LogUsers.nomor_plat, LogUsers.nomor_bpkb, LogUsers.pemilik, LogUsers.jenis_kendaraan, LogUsers.merk, LogUsers.warna, LogUsers.alamat)).all()
        if (log_data is None):
...
class UserAPI(Resource):
    def delete(self, nomor_bpkb):
        ...
        if (users is None):
            return f"Data User dengan NIK {nomor_bpkb} tidak ditemukan!"
        else:
            tilang = users[0]
            db.session.delete(tilang)
            db.session.commit()
            return f"Data User dengan NIK {nomor_bpkb} berhasil dihapus!"
